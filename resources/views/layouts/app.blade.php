...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>{{ config('app.name', 'Rang') }}</title>
        <!-- Favicon -->
        <link href="{{ asset('argon') }}/img/logos/1.png" rel="icon" type="image/png">
        <!-- Fonts -->
    ...
    <body class="{{ $class ?? '' }}">
        ...
        <script>
            ...
            $(document).on("click",".add-btn-color", function(){
                var $this = $(this);  
                var parent = $(this).parent();  
                var val = parent.find(".new-state").val(); 
                var select2 = $('#colors');  
                var token = parent.find('input[name="_token"]').val();
                parent.find('.error-option').remove();
                ...
                jQuery.ajax({
                    ...
                    success: function(results) { 
                        ...
                        if(results.status == 1)
                        {
                            var option = new Option(val, results.id, true, true);
                            select2.append(option).trigger('change');  
                            parent.find(".new-state").val('');
                        }else{
                            $this.after('<span class="error-option" style="color:red;font-size:12px;">'+results.message+'</span>');
                        }
                    }      
                });
                
            });
            $(document).on("click",".add-btn-patterns", function(){
                var $this = $(this);  
                var parent = $(this).parent();  
                var val = parent.find(".new-state").val(); 
                var select2 = $('#patterns');  
                var token = parent.find('input[name="_token"]').val();
                parent.find('.error-option').remove();
