...
class FrontEndController extends Controller
{
    ...
    public function user_request(Request $request)
    {
        ...
        $UserRequest->manufacturer_id = $request->manufacturer_id;
        $UserRequest->image_id = $request->image_id;
        $UserRequest->pattern = $request->pattern; 
        $UserRequest->image_size = $request->image_size; 
        $UserRequest->image_quantity = $request->image_quantity; 
        $UserRequest->save(); 

        if($UserRequest->id)
        {
            return redirect()->route('home2')->withStatus(__('Request sent successfully.'));
        }else{
            return redirect()->route('home2')->withStatus(__('Request not sent successfully'));
        }       
